...
SimpleSchema.extendOptions({
  publish: Match.Optional(Boolean), // public: true means the field is published freely
...
Mongo.Collection.prototype.getCursorJoins = function (cursor) {
  ...
  joins.forEach(join => {
    ...
    const collectionPublicFields = joinCollection.getPublishedFields();
...
Mongo.Collection.prototype.smartPublish = function (publicationName, publicationOptions = {}) {
  ...
  Meteor.publish(publicationName, function (terms) {
    ...
    // this.autorun(function (computation) { // disable for now until it works with SSR
      ...
      options.fields = PublicationUtils.arrayToFields(collection.getPublishedFields());
...
Mongo.Collection.prototype.getPublishedFields = function () {
...
Mongo.Collection.prototype.getJoins = function () {
